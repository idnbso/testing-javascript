...
function expect(actual) {
    return {
        toBe: (expected) => { if (expected !== actual) throw new Error(`${result} is not equal ${expected}`) },
        toEqual: (expected) => { if (JSON.stringify(expected) == JSON.stringify(actual)) throw new Error(`${result} is not exactly equal to ${expected}`) },
        toBeGreaterThan: (expected) => { if (expected  >= actual) throw new Error(`${result} is not greater than ${expected}`) },
        toBeLessThan: (expected) => { if (expected <= actual) throw new Error(`${result} is not less than ${expected}`) },
    }
}